...
typedef double dim[PHONE_COUNT][PHONE_COUNT][26];
...
int main(){
	...
	dim* p = new dim[PHONE_COUNT]; //p[41][41][41][26];
	...
	while(1){
		...
		int temp3 = int_map(p3);
		int temp4 = g - 'A';
		//cout<<temp1<<" "<<temp2<<" "<<temp3<<" "<<temp4<<endl;
		p[temp1][temp2][temp3][temp4] = prob;
		//cout<<p[temp1][temp2][temp3][temp4]<<endl;
	}
...
*/
	...
	double seqscore[PHONE_COUNT][20];
	for(int i=0; i<PHONE_COUNT; i++){
		for(int j=0; j<20; j++) seqscore[i][j]=0.0;
	}

	ifstream probfile("temp/init_1.data", ios::in);
	...
	ifstream prob2file("temp/init_2.data", ios::in);
	...
	while(true){
		...
		double backptr[PHONE_COUNT][t];

		int count = 0;
		
		for(int z=2; z<t; z++){

			for (int s = 0; s< PHONE_COUNT; s++){
				double maxnum = 0;
					int bestindex = 0;
				
				for(int q=0; q<PHONE_COUNT; q++){
					
					for(int r=0; r<PHONE_COUNT; r++){
						double temp = seqscore[r][z-2]*p[r][q][s][val[z-2]-'A'];
		...
		for(int i=1; i<t-2; i++) cout << phone_map(c[i]) << " ";
